        ...
        <main class="min-h-screen bg-white px-4 py-10">
            <div class="mx-auto max-w-6xl">
                <div class="mb-8 flex flex-wrap items-center gap-3">
                    ...
                    <Link v-if="$page.props.auth.user?.is_admin" :href="route('blog.create')" class="rounded-full bg-neutral-900 px-4 py-2 text-sm font-semibold text-white">
                        Nuovo post
                    </Link>
                </div>
                    ...
                    <article v-for="post in posts.data" :key="post.id" class="overflow-hidden rounded-xl border bg-neutral-50 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                        ...
                        <div class="p-5">
                            ...
                            <div class="mt-4 flex gap-3">
                                ...
                                <Link v-if="$page.props.auth.user?.is_admin" :href="route('blog.edit', post.id)" class="text-sm font-semibold text-neutral-600">Modifica</Link>
                            </div>
                        </div>
                    </article>
                </div>
